...
import android.view.ContextMenu;//一个上下文菜单
import android.view.Menu;//定义菜单
import android.view.MenuInflater;//实例化Menu目录下的Menu布局文件
import android.view.MenuItem;
import android.view.View;//除了有视图的意义，还包含用户交互和显示
import android.view.ContextMenu.ContextMenuInfo;//ContextMenu的间接子类，用来获取更多的上下文信息
import android.widget.AdapterView;//视图类，用户界面UI组件
import android.widget.ListView;//一个视图组，显示可滚动项目的列表，并将每个项目结果转换为放置到列表中的视图
import android.widget.SimpleCursorAdapter;//将一个Cursor的所有列分别绑定到ImageView或者是TextView

/**
 * Displays a list of notes. Will display notes from the {@link Uri}
 * provided in the incoming Intent if there is one, otherwise it defaults to displaying the
 * contents of the {@link NotePadProvider}.
 *显示注释列表。将显示来自传入意图中提供的{@link Uri}的注释（如果有），否则它默认显示{@linknotepadprovider}的内容。
 * NOTE: Notice that the provider operations in this Activity are taking place on the UI thread.
 * This is not a good practice. It is only done here to make the code more readable. A real
 * application should use the {@link android.content.AsyncQueryHandler} or
 * {@link android.os.AsyncTask} object to perform operations asynchronously on a separate thread.
 * 注意：请注意，此活动中的提供者操作是在UI上进行的线程。这个不是一个好的做法。
 * 这样做只是为了让代码更具可读性。{应用程序应该使用real链接android.content.AsyncQueryHandler}或者
 * {@链接android.os.AsyncTask}对象在单独的线程上异步执行操作。
 */
public class NotesList extends ListActivity {
    ...
    /**
     * The columns needed by the cursor adapter 游标适配器所需的列
     */
